    ...
    return this.accountService.currentUser$.pipe(
      map(auth => {
        if (auth?.accountType === 'Seller') {
          ...
          this.toastrService.info('Need to log in as Seller to access this page', 'Account');
          return false;
        } else {
          console.log('Seller or Admin');
          this.router.navigate(['/product']);
          this.toastrService.warning('Access denied, need to log in with a Seller account instead', 'Access Denied');
          return false;
        }
      })
    );
