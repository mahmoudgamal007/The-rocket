    ...
    return this.accountService.currentUser$.pipe(
      map(auth => {
        if (auth?.accountType === 'Admin') {
          ...
          this.toastrService.info('Need to log in as Admin to access this page', 'Account');
          return false;
        } else {
          console.log('Seller or Admin');
          this.router.navigate(['/product']);
          this.toastrService.warning('Access denied, need to log in with an Admin account instead', 'Access Denied');
          return false;
        }
      })
    );
